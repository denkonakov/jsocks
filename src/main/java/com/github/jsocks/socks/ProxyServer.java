package com.github.jsocks.socks;

import com.github.jsocks.socks.server.ServerAuthenticator;
import java.net.*;
import java.io.*;
...
/**
 * SOCKS4 and SOCKS5 proxy, handles both protocols simultaneously. Implements all SOCKS commands,
 * including UDP relaying.
 * 
 * <p>
 * In order to use it you will need to implement ServerAuthenticator interface. There is an
 * implementation of this interface which does no authentication ServerAuthenticatorNone, but it is
 * very dangerous to use, as it will give access to your local network to anybody in the world. One
 * should never use this authentication scheme unless one have pretty good reason to do so. There is
 * a couple of other authentication schemes in com.github.jsocks.socks.server package.
 * 
 * @see com.github.jsocks.socks.server.ServerAuthenticator
 */
public class ProxyServer implements Runnable
{
	...
	Logger log_ = Logger.getLogger(ProxyServer.class.getName());
	 ...
	 * Set proxy.
	 * <p>
	 * Allows Proxy chaining so that one Proxy server is connected to another and so on. If proxy
	 * supports SOCKSv4, then only some SOCKSv5 requests can be handled, UDP would not work, however
	 * CONNECT and BIND will be translated.
	 * 
	...
	/**
	 * Sets the timeout for connections, how long shoud server wait for data to arrive before
	 * dropping the connection.<br>
	...
	/**
	 * Sets the timeout for BIND command, how long the server should wait for the incoming
	 * connection.<br>
	 * Zero timeout implies infinity.<br>
	 * Default timeout is 3 minutes.
	 */
	...
	public void start(int port, int backlog, InetAddress localIP)
	{
		try
		{
			serverSocket_ = new ServerSocket(port, backlog, localIP);
			log_.info("Starting com.github.jsocks.SOCKS Proxy on:" + serverSocket_.getInetAddress().getHostAddress()
			...
			new Thread(new Runnable()
			{
				...
				public void run()
				{
					while (!stopped)
					{
						...
						try
						{
							s = serverSocket_.accept();
						}
						catch (IOException e)
						{
							log_.throwing(this.getClass().getName(), "start()", e);
						}

						if (s != null)
						{
							log("Accepted from:" + s.getInetAddress().getHostName() + ":" + s.getPort());
							executor_.submit(new ProxyServer(auth, s));
						}
					}
				}
			}).start();
	...
	public void stop()
	{
		...
		{
			log_.throwing(this.getClass().getName(), "stop()", ioe);
		}
	}
	...
	public void run()
	{
		switch (mode)
		{
			...
			case PIPE_MODE:
				...
				{
					log_.throwing(this.getClass().getName(), "run() {PIPE_MODE}", ioe);
				}
				finally
				{
					abort();
...
/////////////////
	...
	private void onBind(ProxyMessage msg) throws IOException
	{
		...
		if (proxy == null)
		{
			...
			serverSocket_ = new SocksServerSocket(proxy, msg.ip, msg.port);

		serverSocket_.setSoTimeout(acceptTimeout);

		log("Trying accept on " + serverSocket_.getInetAddress() + ":" + serverSocket_.getLocalPort());

		if (msg.version == 5)
			response = new Socks5Message(Proxy.SOCKS_SUCCESS, serverSocket_.getInetAddress(),
				serverSocket_.getLocalPort());
		else
			response = new Socks4Message(Socks4Message.REPLY_OK, serverSocket_.getInetAddress(),
				serverSocket_.getLocalPort());
		response.write(out);
		...
		catch (InterruptedIOException iioe)
		{
			...
			if (mode != PIPE_MODE)
				return;//If accept thread was not successful return.
		}
	...
	private void onUDP(ProxyMessage msg) throws IOException
	{
		...
		catch (EOFException iof)
		{
			log_.throwing(this.getClass().getName(), "onUDP()", iof);
		}
	}
...
//////////////////
	...
	private void doAccept() throws IOException
	{
		...
		while (true)
		{
			...
			else if (serverSocket_ instanceof SocksServerSocket)
			{
	...
	private void startPipe(Socket s)
	{
		...
		{
			log_.throwing(this.getClass().getName(), "startPipe()", ioe);
		}
	}
	...
	private void sendErrorMessage(int error_code)
	{
		...
		{
			log_.throwing(this.getClass().getName(), "sendErrorMessage()", ioe);
		}
	}
	...
	private synchronized void abort()
	{
		...
		{
			log_.throwing(this.getClass().getName(), "abort()", ioe);
		}
	}
	...
	{
		log("Request version:" + msg.version + "\tCommand: " + command2String(msg.command));
		log("IP:" + msg.ip + "\tPort:" + msg.port + (msg.version == 4 ? "\tUser:" + msg.user : ""));
	}
