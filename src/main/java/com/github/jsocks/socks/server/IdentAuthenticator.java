...
import com.github.jsocks.socks.InetRange;
...
/**
  An implementation of com.github.jsocks.socks.ServerAuthentication which provides
...
//////////////////
  private int getRangeIndex(InetAddress ip){
     int index = 0;
     Enumeration elements = hosts.elements();
     while(elements.hasMoreElements()){
       InetRange ir = (InetRange) elements.nextElement();
  ...
  private String userNames(int i){
    ...
    Enumeration element = ((Hashtable)users.elementAt(i)).keys();
    if(!element.hasMoreElements()) return "";
    String s = element.nextElement().toString();
    while(element.hasMoreElements())
       s += "; "+element.nextElement();
