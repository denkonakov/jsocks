...
import com.github.jsocks.socks.Proxy;
...
public class TestClient extends TestService
{
	...
	/** Host on which com.github.jsocks.stubs.TestServer is running */
	String testHost;
	...
	void testEcho() throws IOException
	{
		...
		for (int i = 0; i < 5; ++i)
		{
			...
			out.flush();
			log("com.github.jsocks.EchoTest:" + in.readLine());
			;
		}
		log("com.github.jsocks.EchoTest finished");
	}
	...
	public void testAccept(int service, boolean useString) throws IOException
	{
		...
		if (useString)
			...
			ss = new SocksServerSocket(proxy, InetAddress.getByName(testHost), servicePorts[service]);
