package com.github.jsocks.stubs;

import java.io.*;
import java.net.*;

/**
 * Server to used perform tests for com.github.jsocks.SOCKS library.
 */
public class TestService implements Runnable
{
	...
	public static final String serviceNames[] = {"echo", "discard", "chargen", "connect"};
    public static final int servicePorts[] = {5678, 5679, 5680, 5681};

    public static final int ECHO = 0;
    public static final int DISCARD = 1;
    public static final int CHARGEN = 2;
	...
	/**
	 * Creates new com.github.jsocks.stubs.TestService object, which will perform particular service
	 * on given socket.
	 * 
	 ...
	 * @return Integer identifier for this servuce, or -1, if service can't be found.
	 */
	...
	/**
	 * Echos any input on the socket to the output. com.github.jsocks.EchoTest is being done line by
	 * line.
	 * 
	 * @param s Socket on which to perform service.
	 */
	static public void echo(Socket s) throws IOException
	{
		...
		log("com.github.jsocks.EchoTest done.");
	}

	/**
	 * Reads input from the socket, and does not write anything back. logs input in line by line
	 * fashion.
	 * 
	 * @param s Socket on which to perform service.
	 */
	 ...
	 * <p>
	 * Unlike usual chargen (port 19), each next line of the generated output is send after
	 * increasingly larger time intervals. It starts from wait_time (ms), and each next time wait
	 * time is doubled. Eg. 1 2 4 8 16 32 64 128 256 512 1024 2048 4096 8192 ... well you got the
	 * idea.
	 * <p>
	 * It starts if either connection is clsoed or the wait time grows bigger than max_wait.
	 * 
	 * @param s Socket on which to perform service.
	 * @param wait_time Time in ms, from which timing sequence should begin.
	 * @param max_wait Time in ms, after reaching timeout greater than this value, chargen will
	 *        stop.
	 */
	 ...
	 * <p>
	 * Reads a line from the socket connection, line should be in the form port service_id. Connects
	 * back to the remote host to port specified in the line, if successful performs a service
	 * speciefied by id on that new connection. If accept was successful closes the control
	 * connection, else outputs error message, and then closes the connection.
	 * 
